...
protected:
    ...
    std::vector<TimeSeriesData> create_data(int count, int64_t start_time, int64_t interval = 1000) {
        ...
        for (int i = 0; i < count; ++i) {
            data.push_back({start_time + i * interval, static_cast<double>(i + 1)});
        }
        return data;
    }
};
...
TEST_F(WindowAggregatorTest, SumAggregation) {
    ...
    EXPECT_DOUBLE_EQ(results[0].value, 55.0);
}
...
TEST_F(WindowAggregatorTest, AverageAggregation) {
    ...
    EXPECT_DOUBLE_EQ(results[0].value, 5.5);
}
...
TEST_F(WindowAggregatorTest, StdDevAggregation) {
    ...
    std::vector<TimeSeriesData> data;
    for (int i = 0; i < 10; ++i) {
        data.push_back({base_time + i * 1000, 5.0}); // All same value
    }
    ...
    EXPECT_NEAR(results[0].value, 0.0, 1e-6);
}
...
TEST_F(WindowAggregatorTest, SingleDataPoint) {
    ...
    std::vector<TimeSeriesData> data = {{base_time, 42.0}};
    auto results = agg.process(data);
...
TEST_F(WindowAggregatorTest, WindowAlignment) {
    WindowAggregator agg;
    agg.configure({
        {"window_type", "tumbling"},
        {"window_size", "10000"},
    ...
    auto data = create_data(15, base_time);
    auto results = agg.process(data);
    
    // Verify window timestamps are aligned
    for (const auto& result : results) {
        EXPECT_EQ(result.timestamp % 10000, 0);
    }
}
...
TEST_F(WindowAggregatorTest, Statistics) {
    ...
    EXPECT_GT(stats.at("total_windows_processed"), 0);
    EXPECT_GT(stats.at("total_data_points_aggregated"), 0);
}
