...
def create_comparison_bar_chart():
    ...
    ax.set_title(
        "PECJ Benchmark Fine-Grained Timing Comparison\n(20,000 events, 387 windows, 13,856 joins)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def create_stacked_bar_chart():
    ...
    for i, phase in enumerate(phases):
        ...
        ax.bar(
            0,
            integrated_percent[i],
            bottom=bottom_integrated,
            label=phase if i < len(phases) else "",
            color=color,
            edgecolor="white",
            linewidth=2,
        )
        ...
        if integrated_percent[i] > 3:
            ax.text(
                0,
                bottom_integrated + integrated_percent[i] / 2,
        ...
        # Plugin bar
        ax.bar(
            1, plugin_percent[i], bottom=bottom_plugin, color=color, edgecolor="white", linewidth=2
        )

        if plugin_percent[i] > 3:
            ax.text(
                1,
                bottom_plugin + plugin_percent[i] / 2,
...
def create_speedup_chart():
    ...
    bars = ax2.barh(
        speedup_labels, speedups, color=colors_speedup, alpha=0.8, edgecolor="black", linewidth=1.5
    )
    ...
    ax2.axvline(x=1, color="red", linestyle="--", linewidth=2, alpha=0.7, label="No Change (1x)")
...
def create_summary_table():
    ...
    plt.title(
        "PECJ Benchmark Complete Timing Metrics Comparison\n(20,000 events, 387 windows)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
