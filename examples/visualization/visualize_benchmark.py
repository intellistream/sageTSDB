...
def create_total_time_comparison(ax):
    ...
    bars = ax.bar(
        modes, times, color=colors_list, alpha=0.8, width=0.6, edgecolor="black", linewidth=1.5
    )
    ...
    ax.text(
        ...
        fontweight="bold",
        bbox=dict(
            boxstyle="round,pad=0.5", facecolor="#F39C12", edgecolor="black", linewidth=2, alpha=0.8
        ),
    )
...
def create_bottleneck_analysis(ax):
    ...
    contributions = {
        ...
        "Result Writing\nOptimization": INTEGRATED_MODE["Result Writing"]
        - PLUGIN_MODE["Result Writing"],
        "Data Prep\nOverhead": -(
            PLUGIN_MODE["Data Preparation"] - INTEGRATED_MODE["Data Preparation"]
        ),
    ...
    bars = ax.barh(labels, values, color=bar_colors, alpha=0.8, edgecolor="black", linewidth=1.2)
    ...
    legend_elements = [
        ...
        mpatches.Patch(
            facecolor=colors["negative"], label="Negative (Slowdown)", edgecolor="black"
        ),
    ]
...
def create_speedup_summary(ax):
    ...
    bars = ax.barh(phases, speedups, color=bar_colors, alpha=0.8, edgecolor="black", linewidth=1.2)
    ...
    ax.axvline(x=1, color="red", linestyle="--", linewidth=2, alpha=0.7, label="No Change (1×)")
...
def main():
    ...
    # Add overall title
    fig.suptitle(
        "PECJ Benchmark: Integrated vs Plugin Mode - Fine-Grained Analysis\n"
    ...
    fig.text(
        ...
        fontsize=10,
        bbox=dict(
            boxstyle="round", facecolor="lightyellow", edgecolor="black", linewidth=2, alpha=0.9
        ),
    ...
    print("=" * 70)
    print(
        f"Total Time:     Integrated={INTEGRATED_MODE['Total Time']:.2f}ms  "
        f"Plugin={PLUGIN_MODE['Total Time']:.2f}ms"
    )
    print(f"Speedup:        {INTEGRATED_MODE['Total Time'] / PLUGIN_MODE['Total Time']:.2f}×")
    print(
        f"Data Access:    {INTEGRATED_MODE['Data Access'] / PLUGIN_MODE['Data Access']:.2f}× faster"
    )
    print(
        f"Pure Compute:   {INTEGRATED_MODE['Pure Compute'] / PLUGIN_MODE['Pure Compute']:.2f}× faster"
    )
    print("=" * 70 + "\n")
