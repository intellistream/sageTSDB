...
// ctor + sub-components); used by both the compute engine and the adapter.
    ...
    try {
        ...
        switch (op_type) {
            ...
            case PECJOperatorType::MSWJ:
                ...
                // construction leaves null). The shared helper also calls
                // setConfig() internally, so skip the shared setConfig() below.
                pecj_operator_ = detail::createMSWJOperator(pecj_config);
